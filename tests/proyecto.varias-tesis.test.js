...
/**
 * Varias tesis del mismo método, solo para administradores.
 *
 * Lo que se prueba: que el panel enseña UNA fila por método —la activa, la
 * misma que usa el conector— con la lista de todas; que solo al administrador
 * se le ofrece abrir otra; que abrirla exige licencia; y que la última tesis no
 * se borra por este camino. La base y el disco se sustituyen.
 */

const test = require('node:test');
const assert = require('node:assert/strict');

const estado = { guardados: [], conLicencia: [], cuantas: 0, pasos: [], existe: true };
...
function empezar(cambios = {}) {
  Object.assign(estado, { guardados: [], conLicencia: [], cuantas: 0, pasos: [], existe: true }, cambios);
}
...
test('con varias tesis, el panel enseña una fila: la activada más tarde', async () => {
  ...
  const panel = await projectService.deUsuario('u1', { variasTesis: true });
...
test('a un comprador no se le ofrece abrir otra tesis', async () => {
...
test('un método comprado sin nada guardado sale con la lista vacía', async () => {
  ...
  const [metodo] = await projectService.deUsuario('u1', { variasTesis: true });
  assert.equal(metodo.guardado, false);
  assert.deepEqual(metodo.tesis, []);
});

test('abrir otra tesis exige licencia vigente de ese método', async () => {
  empezar({ conLicencia: ['OTRO'] });
  const sin = await projectService.crearTesis({ userId: 'u1', productCode: 'METODO', nombre: 'X' });
  assert.equal(sin, null);
  assert.deepEqual(estado.pasos, []);

  empezar({ conLicencia: ['METODO'] });
  const con = await projectService.crearTesis({ userId: 'u1', productCode: 'METODO', nombre: 'X' });
  assert.equal(con.id, 'nueva');
  assert.deepEqual(estado.pasos, ['crear:X']);
});
